...
SUMO_CONFIG = r"Simulation Files\simulation.sumocfg"  # Use raw string for Windows path
sumo_tools_path = os.path.join(SUMO_HOME, 'tools')  # Path to the SUMO tools directory
net_file = r'Simulation Files\reduced_network.net.xml'  # Use raw string for Windows path
routes_file = r'Simulation Files\randomRoutes.rou.xml'  # Use raw string for Windows path
trips_file = r'Simulation Files\randomTrips.trips.xml'  # Use raw string for Windows path
...
class SumoEnv(Env):
    def __init__(self):
        ...
        self.numIntersections=5
        self.intersections = ['b','d','e','f','h']
        self.action_space = MultiDiscrete([16]*5)  # Observations: Number of vehicles, queue lengths, etc., for each intersection
        self.observation_space = Box(low=0, high=np.inf, shape=(15,), dtype=np.float32)
        self.stepCount=0
...
class SumoEnv(Env):
    def __init__(self):
        ...
        self.intersections = ['b','d','e','f','h']
        self.action_space = MultiDiscrete([16]*5)  # Observations: Number of vehicles, queue lengths, etc., for each intersection
        self.observation_space = Box(low=0, high=np.inf, shape=(10,), dtype=np.float32)
        self.stepCount=0
